...
#SOURCE_DATA = "../neuroData/domain_patterns/terminology_data/lasso_entity_data_freq_label_labelled_labelled_corrected.csv"
...
def add_label(attrname, old, new):
    ...
    source.data = df
    proportion = 100 * (
        df.loc[~df["label"].isna(), "frequency"].sum() / df["frequency"].sum()
    )
